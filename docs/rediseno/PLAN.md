...
**Imágenes (colocadas el 2026-09-20):** las 140 imágenes generadas (14 del sitio, 20 portadas, 106 esquemas explicativos) están en `public/images/site/` y `public/images/guias/<categoria>/<slug>/`, asignadas con el «Mapa de asignación» de `PROMPTS-IMAGENES.md`. Las 90 `prueba-prompt-0N.webp` siguen vacías a propósito (capturas reales del autor).
- **Sitio (PNG transparente):** se recortó el margen transparente de los originales (2,5 % de respiro para la sombra, máx. 1600 px) y `SiteImage` lee el tamaño real de la cabecera del PNG (`pngSize` en `lib/guides/media.ts`), así que cualquier proporción funciona y un archivo nuevo no exige tocar código. Si se reemplaza una, conviene recortar su margen igual.
- **Guías (WebP):** se convirtieron con calidad 88 a 1600 px de ancho máximo.
- **Corrección de 15 esquemas:** E004, E005, E011, E017, E028, E051, E059, E064, E065, E068, E073, E074, E077, E082 y E090 llegaron con las celdas «resaltadas» como rectángulos negros macizos (defecto de generación). Se repintaron con un tono claro tomado del color de su propio borde (coral, violeta o menta). Los originales siguen intactos en la carpeta de descarga.
- **Ilustración, no captura:** los esquemas explicativos no llevan texto ni cifras legibles. Por eso `slotToImage` los marca `illustration` (salvo `prueba-prompt-*`, `promptId` o `real: true` en el slot): `ImageBlock` añade el rótulo «Ilustración», antepone «Esquema ilustrativo.» al alt, no ofrece ampliar y limita el ancho a 42 rem. Si el autor sustituye uno por una captura real, basta `real: true` en su slot.
...
- **SEO:** `og:image` y el `image` del JSON-LD de cada guía apuntan ahora a su `hero.webp`.
