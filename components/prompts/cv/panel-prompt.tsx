...
export function PanelPrompt({ alCopiar }: { alCopiar: () => void }) {
  ...
  return (
    <section aria-labelledby="titulo-prompt" className="tarjeta p-5 sm:p-6">
      <h2 id="titulo-prompt" className="text-lg font-semibold leading-tight">
        2. Tu prompt <span className="font-normal text-muted-foreground">(se arma solo)</span>
      </h2>
      ...
      <div className="mt-5" role="group" aria-label="Avance del formulario">
        <div className="flex items-center justify-between text-sm">
          ...
          <span className="tabular text-muted-foreground">{porcentaje} %</span>
        </div>
        <div className="mt-2 h-2 overflow-hidden rounded-full bg-surface" aria-hidden>
          <div className="h-full rounded-full bg-brand-solid transition-all duration-200" style={{ width: `${porcentaje}%` }} />
        </div>
      ...
      <pre
        ...
        data-prompt
        className="mt-4 max-h-[26rem] overflow-auto whitespace-pre-wrap break-words rounded-lg border bg-surface p-4 font-mono text-[0.8125rem] leading-relaxed"
      >
        {prompt}
      </pre>
